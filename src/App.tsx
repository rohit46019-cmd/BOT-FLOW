  ...
  const handleSendCode = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const data = await res.json();
        showNotification('error', data.error || 'Failed to send code');
      }
    } catch (err) {
      showNotification('error', 'Connection error');
    } finally {
  ...
  const handleSignIn = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const data = await res.json();
        showNotification('error', data.error || 'Sign in failed');
      }
    } catch (err) {
      showNotification('error', 'Connection error');
    } finally {
                  ...
                  <div className="space-y-6">
                    ...
                    {authStep === 'phone' && (
                      <div className="space-y-4">
                        ...
                        <button onClick={() => setAuthStep('credentials')} className="w-full text-slate-500 text-sm">Back</button>
                      </div>
                    )}
