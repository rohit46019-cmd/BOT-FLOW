...
async function test() {
  ...
  try {
    // We already created a topic earlier, let's get its ID.
    // Or just create a new one and then edit it.
    const createResult = await client.invoke(
      new Api.channels.CreateForumTopic({
        channel: targetGroup,
        title: "Test Edit Topic Icon",
        randomId: Math.floor(Math.random() * 1000000),
      })
    );
    
    let topicId = 0;
    if (createResult instanceof Api.Updates && createResult.updates.length > 0) {
      for (const update of createResult.updates) {
    ...
    if (topicId > 0) {
      const editResult = await client.invoke(
      ...
      console.log("Edited topic:", editResult);
    }
